...
class Device(DeviceResult):
    cdc_path: str | None
    usb_id: str
    name: str


DeviceInfoType = collections.defaultdict[str, collections.defaultdict[str, Device]]


def get_dev_udevadm_path_map() -> dict[str, str]:
    result: dict[str, str] = {}

    dev_paths = subprocess.run(["find", "/dev"], stdout=subprocess.PIPE).stdout.decode().splitlines()  # nosec B603, B607
...
def list_usb_devices() -> list[Device]:
    ...
    dev_infos: list[DeviceResult] = [
        ...
        for z in subprocess.run(args=["find", device_base_path], stdout=subprocess.PIPE).stdout.decode().splitlines()  # nosec B603
    ...
    lsusb_infos: list[LSUSBResult] = [
        ...
        for z in subprocess.run(args=["lsusb"], stdout=subprocess.PIPE).stdout.decode().splitlines()  # nosec B603
