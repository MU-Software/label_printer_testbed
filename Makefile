...
IMAGE_NAME := $(if $(IMAGE_NAME),$(IMAGE_NAME),label_printer_testbed_image)
...
# Usage: make docker-build <image-name:=label_printer_testbed_image>
# if you want to build with debug mode, set DOCKER_DEBUG=true
# ex) make docker-build label_printer_testbed_image DOCKER_DEBUG=true
docker-build:
	docker build \
		-f ./infrastructure/Dockerfile -t $(IMAGE_NAME) \
